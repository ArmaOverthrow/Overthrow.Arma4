...
**5a. Implement.** Spawn the best agent at the plan's tier with a tight prompt: the phase's tasks, the exact files, the acceptance criteria from `implementation.md`, and the relevant decisions/gotchas from `context.md`. Tell the agent to run the project's type-check/build and not to exceed the phase scope. **The agent's gate is the cheap headless one only — include this line verbatim in the prompt: "Do not run `tools/run-tests.sh`. Your gate is `tools/compile-check.sh` exit 0 — I run the test suites myself after the phase completes."** **Every implementation agent prompt MUST include the no-git rule verbatim: "Do not run any git command that writes state — no branch, checkout, add, commit, stash, merge, rebase, pull, or push. Leave all changes uncommitted in the working tree."** If an agent commits anyway, do not try to undo it — report it in the final summary and let the user decide.

**5b. Automated gate (must pass before continuing).** **You run this yourself, in the main thread — never a subagent.**
- Run the project's type-check / lint / build in every package or area touched (here: `tools/compile-check.sh`, plus the input-conflict checker for UI phases). Cheap and headless — re-run freely until clean.
- **Then, once, the regression gate: `tools/run-tests.sh`.** Read `.claude/test-policy.md` and obey it — it is the single source of truth for when the suites may run. In short: the suites launch a real Reforger client that **steals the user's desktop focus for ~15–20 s**, so it is one run per completed phase, by you, announced before it launches; **Fast group by default**, All only if the phase touched campaign/economy/persistence state; **skip it entirely** (and say so) when the phase was docs/layout/prefab/localization-only, since the suites assert nothing there; **defer it** if the user may be play-testing or a launcher session from this conversation is still running.
- For UI phases, the suites cover nothing — do not substitute a test run for the MCP/manual check in 5c.
- Check the relevant dev log for compilation errors / runtime exceptions introduced by the phase.
- **If the gate fails:** these suites are deterministic (`maxAttempts` is banned project-wide), so a red is real — never re-run to see if it sticks. Read the failing case names and `.tmp/run-tests/` artifacts, then either fix it yourself in the main thread or send the agent back with the case names (~2–3 focused iterations), and re-run **once**. If it is still red, or the fix is not apparent, STOP and prompt with the exact failures. Exit 2 is *no verdict*, not a pass — never report it as green.
