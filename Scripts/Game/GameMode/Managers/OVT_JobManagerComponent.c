...
class OVT_JobManagerComponent: OVT_Component
{
	...
	//------------------------------------------------------------------------------------------------
	//! Destructor for the OVT_JobManagerComponent. Cleans up call queue and clears collections.
	void ~OVT_JobManagerComponent()
	{
		if (GetGame() && GetGame().GetCallqueue()) // Ensure callqueue exists before removing
		{
		...
		if(m_aTownJobs)
		{
			// Need to clear the inner sets too if they contain objects (they contain ints here, so Clear() is ok)
			m_aTownJobs.Clear();
			m_aTownJobs = null;
		}
		if(m_aBaseJobs)
		...
		if(m_aJobs)
		{
			// Clear the array of OVT_Job objects
			m_aJobs.Clear();
			m_aJobs = null;
		}
		if(m_aJobCounts)
		...
		if(m_mPlayerJobCounts)
		{
			// Need to clear inner maps
			for(int i = m_mPlayerJobCounts.Count() - 1; i >= 0; i--)
			{
				map<int, int> innerMap = m_mPlayerJobCounts.GetElement(i);
				if (innerMap) innerMap.Clear();
			}
			m_mPlayerJobCounts.Clear();
			m_mPlayerJobCounts = null;
		}

		// Clear the singleton instance if this is the one being destructed
		if(s_Instance == this)
			s_Instance = null;
	}
}