...
//------------------------------------------------------------------------------------------------
//! The AI horn watchdog must end a LATCH without ever cutting a real honk short.
//!
//! `SetVehicleHorn` has no caller anywhere in the game - the press and the release are both native -
//! so when the engine forgets the release, an AI driver sounds his horn until the character is
//! deleted. Two triggers are on record, both while a driver is failing to make progress, and neither
//! Overthrow's LOD pin nor the vehicle driving deltas turned out to be the cause (both were removed
//! 2026-08-24 and it still latched).
//!
//! Both halves of the contract are asserted, and the ORDER matters: the "a short honk survives" half
//! is checked FIRST, because a watchdog that simply zeroed the horn on every tick would satisfy the
//! release half on its own and be worse than the bug.
//!
//! PROVEN ABLE TO FAIL 2026-08-25: raising OVT_VehicleHornWatchdog.MAX_HELD_MS above the total time
//! this case ticks makes the release assertion go red; returning `held` unconditionally (never
//! writing the flag back) does the same.
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_InitSuite, timeoutS: 60)]
class OVT_TEST_Init_Deployments_HornWatchdogReleasesOnlyALatch : SCR_AutotestCaseBase
{
	//! Comfortably smaller than MAX_HELD_MS, so several of them are still a legitimate honk.
	...
	bool Execute()
	{
		...
		if (!spawnLogic || spawnLogic.m_rDefaultPrefab.IsEmpty())
		{
			SetFailure("No spawn logic instance (or no default character prefab) - there is no driver to give a horn");
			return true;
		}
		...
		CharacterInputContext input = OVT_VehicleHornWatchdog.ResolveInput(m_Character);
		if (!input)
		{
			SetFailure("The spawned character has no input context - the watchdog can neither read nor release a horn, and every assertion below would be vacuous");
			return FinishAndCleanUp();
		}

		// A driver who is not honking must never accumulate, or a series of unrelated ticks would
		// eventually "release" a horn nobody pressed.
		//
		// ⚠ THE BASELINE IS NOT ZERO. A character with no vehicle context reads **-1** here, which is
		// why the watchdog tests for a POSITIVE value rather than `!= 0` - an `!= 0` test counts every
		// man on foot as honking. That is exactly the defect this assertion caught on 2026-08-25.
		if (input.GetVehicleHorn() > 0)
		{
			SetFailure("A freshly spawned character reads horn %1 - it is already holding one, so the baseline is not clean", input.GetVehicleHorn().ToString());
			return FinishAndCleanUp();
		}

		if (OVT_VehicleHornWatchdog.Tick(m_Character, STEP_MS, 0) != 0)
		{
			SetFailure("The watchdog accumulated time against a driver whose horn is OFF");
			return FinishAndCleanUp();
		}

		if (OVT_VehicleHornWatchdog.Tick(null, STEP_MS, 0) != 0)
		{
			SetFailure("The watchdog accumulated time against a null driver - an empty seat would eventually trip it");
			return FinishAndCleanUp();
		}

		// HALF ONE, FIRST: a real honk survives. Held for one step short of the cap, the flag must
		// still be set - a watchdog that zeroed it here would silence every courtesy honk in the game.
		input.SetVehicleHorn(1);

		int held = 0;
		while (held + STEP_MS < OVT_VehicleHornWatchdog.MAX_HELD_MS)
		{
			held = OVT_VehicleHornWatchdog.Tick(m_Character, STEP_MS, held);

			if (input.GetVehicleHorn() == 0)
			{
				SetFailure("The watchdog released the horn after only %1 ms - a legitimate honk is being cut short", held.ToString());
				return FinishAndCleanUp();
			}

			if (held == 0)
			{
				SetFailure("The watchdog reset its accumulator while the horn was still held - it can never reach the cap, so a latch would sound forever");
				return FinishAndCleanUp();
			}
		}

		// A honk that stops on its own must clear the accumulator, so short honks never add up.
		input.SetVehicleHorn(0);
		if (OVT_VehicleHornWatchdog.Tick(m_Character, STEP_MS, held) != 0)
		{
			SetFailure("Releasing the horn did not reset the accumulator - consecutive short honks would accumulate into a spurious release");
			return FinishAndCleanUp();
		}

		// HALF TWO: the latch. Held past the cap, the flag must be written back to zero.
		input.SetVehicleHorn(1);

		held = 0;
		int guard = 0;
		while (input.GetVehicleHorn() != 0 && guard < 100)
		{
			held = OVT_VehicleHornWatchdog.Tick(m_Character, STEP_MS, held);
			guard++;
		}
		...
		if (input.GetVehicleHorn() != 0)
		{
			SetFailure("The watchdog never wrote the horn back to zero - a latched AI horn still sounds until its character is deleted");
			return FinishAndCleanUp();
		}

		if (held != 0)
		{
			SetFailure("The watchdog released the horn but returned a non-zero accumulator (%1) - the next honk would start part-way to the cap", held.ToString());
			return FinishAndCleanUp();
		}

		Print("The horn watchdog leaves a honk alone right up to its cap and writes the flag back once it is past it");
		return FinishAndCleanUp();
	}
