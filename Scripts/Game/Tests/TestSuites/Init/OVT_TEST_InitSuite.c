...
class OVT_TEST_Init_Persistence_ReservationReplicatesToClients : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		{
			SetFailure("Reserve() hid the vehicle but its physics body still has interaction layer %1 - the invisible car still collides (BUG-185)", phys.GetInteractionLayer().ToString());
			return FinishAndCleanUp();
		}
