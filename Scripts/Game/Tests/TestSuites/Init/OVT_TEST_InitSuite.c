...
//!     manager's attribute (BUG-187 - the one reservable prefab the original fix missed);
...
//!  3. the COLLISION half (BUG-185) - Reserve() zeroes the authority body's interaction layer
...
class OVT_TEST_Init_Persistence_ReservationReplicatesToClients : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		// The collision half (BUG-185) is pinned against the authority's own body, which is the one
		...
		if (!phys)
		{
			SetFailure("The spawned vehicle has no Physics body - the BUG-185 collision half cannot be pinned");
			return FinishAndCleanUp();
		}
		...
		{
			SetFailure("Reserve() hid the vehicle but its physics body is still in the physics world (%1) - the invisible car still collides (BUG-185)", typename.EnumToString(SimulationState, phys.GetSimulationState()));
			return FinishAndCleanUp();
		}
		...
		// Wiring, recruit half (BUG-187): reserved recruit bodies go through the same Reserve()
		// path, and the recruit prefab was the one reservable prefab BUG-181's fix missed. Reached
		...
		{
			SetFailure("The recruit prefab (%1) has no OVT_ReservationSyncComponent - a parked recruit's body is a visible ghost on every client (BUG-187)", recruits.m_sRecruitPrefab);
			return FinishAndCleanUp();
		}

		Print("Reservation state reaches the clients: prefab wiring intact on the vehicle, character and recruit chains, Reserve()/Release() drive the replicated flag both ways, and the physics body leaves and re-enters collision with them (BUG-185)");
		return FinishAndCleanUp();
	}
