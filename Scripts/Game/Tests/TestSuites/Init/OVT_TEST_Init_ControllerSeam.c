...
//!      plus OVT_FuelRequestComponent from economy/fuel amendment A1, OVT_HighCommandRequestComponent
//!      from resistance/high-command Phase 1 and OVT_ResourceProductionRequestComponent from
//!      logistics/resource-production Phase 3 - one line per new component is the whole cost of this
//!      guarantee);
...
class OVT_TEST_Init_Controller_ComponentsResolve : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		PrintFormat("Controller seam: all 12 asserted controller components resolve through OVT_ControllerComponent<T>.Get() (controller found after %1 poll(s))", m_iPolls.ToString());
		return true;
	}
	...
	protected string FindFirstUnresolvedComponent()
	{
		...
		if (!OVT_ControllerComponent<OVT_ResourceProductionRequestComponent>.Get()) return "OVT_ResourceProductionRequestComponent";
		if (!OVT_ControllerComponent<OVT_OptionsRequestComponent>.Get()) return "OVT_OptionsRequestComponent";
