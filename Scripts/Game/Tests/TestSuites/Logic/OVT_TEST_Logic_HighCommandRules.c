...
//------------------------------------------------------------------------------------------------
//! An owner ALREADY over the cap is refused, including a purchase that would add nothing. A lowered
//! cap and a recruit conversion both produce that state, and the ladder must not let it grow.
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_LogicSuite, timeoutS: 30)]
class OVT_TEST_Logic_HighCommandRules_CapRefusesWhenAlreadyOver : SCR_AutotestCaseBase
{
	//------------------------------------------------------------------------------------------------
	[TestStep(TestStage.Main)]
	bool Execute()
	{
		if (OVT_HighCommandRules.FitsUnderCap(60, 1, 48))
		{
			SetFailure("An owner already 12 members over a 48 cap was allowed to buy one more");
			return true;
		}

		if (OVT_HighCommandRules.FitsUnderCap(60, 0, 48))
		{
			SetFailure("An owner already over the cap was admitted for a ZERO-member purchase - the test is on the resulting total, not on the increment");
			return true;
		}

		if (!OVT_HighCommandRules.FitsUnderCap(48, 0, 48))
		{
			SetFailure("An owner sitting exactly on the cap was refused a zero-member purchase - exactly on the cap is still under it");
			return true;
		}

		Print("High Command cap: an owner at or over the cap can only be admitted for a purchase that keeps the total at or under it");
		return true;
	}
}

//------------------------------------------------------------------------------------------------
//! At the SHIPPED default rate of one supporter per member, the draw-down is the member count - the
//! number the purchase ladder hands to the town gate and then to TakeSupportersFromNearestTown.
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_LogicSuite, timeoutS: 30)]
class OVT_TEST_Logic_HighCommandRules_SupportersAtDefaultRate : SCR_AutotestCaseBase
{
	//------------------------------------------------------------------------------------------------
	[TestStep(TestStage.Main)]
	bool Execute()
	{
		if (OVT_HighCommandRules.SupportersRequired(7, 1) != 7)
		{
			SetFailure("A 7-member group at the default rate of 1 required %1 supporters, expected 7", OVT_HighCommandRules.SupportersRequired(7, 1).ToString());
			return true;
		}

		if (OVT_HighCommandRules.SupportersRequired(1, 1) != 1)
		{
			SetFailure("The smallest real purchase - one member at rate 1 - required %1 supporters, expected 1", OVT_HighCommandRules.SupportersRequired(1, 1).ToString());
			return true;
		}

		if (OVT_HighCommandRules.SupportersRequired(-4, 1) != 0)
		{
			SetFailure("A negative member count required %1 supporters, expected 0 - a group that does not exist may never credit a town", OVT_HighCommandRules.SupportersRequired(-4, 1).ToString());
			return true;
		}

		Print("High Command supporters: at the default rate of one per member the draw-down is the member count, and never negative");
		return true;
	}
}
