...
//------------------------------------------------------------------------------------------------
//! The tent half of the same composition (implementation.md §3.8): a recruit cost plus a two-line
//! gear manifest, exactly the shape OVT_RecruitCommandComponent.ValidateTentPurchase builds. Stock
//! on one line only must still lower the total.
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_LogicSuite, timeoutS: 30)]
class OVT_TEST_Logic_ItemSourcing_TentDiscountLowersChargedSubtotal : SCR_AutotestCaseBase
{
	//------------------------------------------------------------------------------------------------
	[TestStep(TestStage.Main)]
	bool Execute()
	{
		int recruitCost = 300;
		float feeMultiplier = 1.5;

		array<ref OVT_ItemSourcingLine> manifest = new array<ref OVT_ItemSourcingLine>();

		OVT_ItemSourcingLine rifle = new OVT_ItemSourcingLine();
		rifle.m_sResource = "rifle";
		rifle.m_iNeeded = 1;
		rifle.m_iUnitPrice = 800;
		manifest.Insert(rifle);

		OVT_ItemSourcingLine mags = new OVT_ItemSourcingLine();
		mags.m_sResource = "mag_556";
		mags.m_iNeeded = 6;
		mags.m_iUnitPrice = 25;
		manifest.Insert(mags);

		int coveredUnits;
		int coveredValue;
		int chargedUnits;

		// No warehouse in range: nothing is covered.
		array<int> noStock = {0, 0};
		int chargedDry = OVT_ItemSourcingRules.SplitCoverage(manifest, noStock, coveredUnits, coveredValue, chargedUnits);
		int totalDry = OVT_RecruitPurchaseRules.TotalPrice(recruitCost, chargedDry, feeMultiplier);

		if (totalDry != 1725)
		{
			SetFailure("With no stock in range the total was %1, expected 1725 (300 recruit + 950 gear x 1.5)", totalDry.ToString());
			return true;
		}

		// The mags are covered, the rifle is not.
		array<int> partialStock = {0, 6};
		int chargedPartial = OVT_ItemSourcingRules.SplitCoverage(manifest, partialStock, coveredUnits, coveredValue, chargedUnits);
		int totalPartial = OVT_RecruitPurchaseRules.TotalPrice(recruitCost, chargedPartial, feeMultiplier);

		if (coveredValue != 150 || totalPartial != 1500)
		{
			SetFailure("With the mags covered the covered value was %1 and the total %2 - expected 150 covered and a total of 1500 (300 recruit + 800 gear x 1.5)", coveredValue.ToString(), totalPartial.ToString());
			return true;
		}

		if (totalPartial >= totalDry)
		{
			SetFailure("Warehouse stock did not lower the tent price at all: %1 covered vs %2 dry", totalPartial.ToString(), totalDry.ToString());
			return true;
		}

		// Everything covered: the tent recruit costs the bare recruit cost, gear is free.
		array<int> fullStock = {1, 6};
		int chargedFull = OVT_ItemSourcingRules.SplitCoverage(manifest, fullStock, coveredUnits, coveredValue, chargedUnits);
		int totalFull = OVT_RecruitPurchaseRules.TotalPrice(recruitCost, chargedFull, feeMultiplier);

		if (totalFull != recruitCost)
		{
			SetFailure("With the whole kit covered the total was %1, expected the bare recruit cost of %2", totalFull.ToString(), recruitCost.ToString());
			return true;
		}

		Print("Item sourcing: the tent's warehouse discount lowers the charged subtotal fed to TotalPrice, and full coverage leaves only the recruit cost");
		return true;
	}
}

//------------------------------------------------------------------------------------------------
//! With nothing in range, the coverage split must reproduce EXACTLY the sum a caller would have
//! charged before this feature existed - the standing regression proof for the tent discount
//! (implementation.md Phase 5 acceptance: "with no warehouse in range the quoted total is
//! byte-identical to today's").
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_LogicSuite, timeoutS: 30)]
class OVT_TEST_Logic_ItemSourcing_TentDiscountNoStockMatchesRawSubtotal : SCR_AutotestCaseBase
{
	//------------------------------------------------------------------------------------------------
	[TestStep(TestStage.Main)]
	bool Execute()
	{
		int needed = 6;
		int unitPrice = 25;
		int rawSubtotal = needed * unitPrice;

		array<ref OVT_ItemSourcingLine> manifest = OVT_TEST_ItemSourcingFixture.OneLine("recruit_kit", needed, unitPrice);
		array<int> noStock = {0};

		int coveredUnits;
		int coveredValue;
		int chargedUnits;
		int chargedSubtotal = OVT_ItemSourcingRules.SplitCoverage(manifest, noStock, coveredUnits, coveredValue, chargedUnits);

		if (chargedSubtotal != rawSubtotal || coveredUnits != 0 || coveredValue != 0)
		{
			SetFailure("With zero stock the charged subtotal was %1 (covered units/value %2), expected the untouched raw subtotal of %3 with nothing covered", chargedSubtotal.ToString(), coveredUnits.ToString() + "/" + coveredValue.ToString(), rawSubtotal.ToString());
			return true;
		}
		...
		int totalThroughCoverage = OVT_RecruitPurchaseRules.TotalPrice(recruitCost, chargedSubtotal, feeMultiplier);
		int totalWithoutCoverage = OVT_RecruitPurchaseRules.TotalPrice(recruitCost, rawSubtotal, feeMultiplier);

		if (totalThroughCoverage != totalWithoutCoverage)
		{
			SetFailure("Routing a zero-stock quote through the coverage split changed the total from %1 to %2 - it must be byte-identical to the pre-Phase-5 price", totalWithoutCoverage.ToString(), totalThroughCoverage.ToString());
			return true;
		}

		Print("Item sourcing: with no warehouse in range the tent quote is byte-identical to the raw subtotal");
		return true;
	}
}
