...
//------------------------------------------------------------------------------------------------
//! PLAN RESOLUTION - which DOCTRINE wins, once selection stopped being one sorted list.
//!
//! WHAT CHANGED AND WHY THIS CASE EXISTS. Until plan-driven selection landed there was one list of
//! every resistance-held place and one comparison. There are now two steps: each plan's selector
//! picks its own best candidate (OVT_ObjectiveSelection.SelectBestIndex, pinned next door), that
//! score is scaled by the plan's own m_fPriority (OVT_ObjectivePlanRules.ResolvePlanScore), and the
//! plans are compared (SelectBestPlanIndex). This case pins the SECOND step and the composition of
//! the two, which is where the fork from one list into two doctrines can go wrong silently: nothing
//! errors, nothing warns, the occupying faction simply attacks a different place.
//!
//! FOUR CLAIMS, AND EACH ONE IS A DIFFERENT WAY THE FORK COULD BREAK:
//!   1. AT EQUAL PRIORITY THE HIGHER RAW SCORE WINS. Both shipped plans author m_fPriority 1, so
//!      this row IS the shipped behaviour, and it is what makes "two plans reproduce the single
//!      list" true: multiplying by one is exact in binary floating point, so a plan's rank is its
//!      selector's score and every comparison is the comparison the single list made.
//!   2. THE MULTIPLIER REALLY MULTIPLIES. A doctrine at 2.0 out-ranks an equally-scored rival and
//!      can out-rank a BETTER-scored one, which is the entire authoring gesture the field exists
//!      for. A multiplier written as an addition would look almost right at small numbers and be
//!      completely wrong at large ones.
//!   3. ZERO EXCLUDES, AND NEGATIVE IS FLOORED. "Ship this doctrine but never run it" is a
//!      supported gesture; a negative multiplier is a mis-key, and without the floor a plan could
//!      out-rank every rival by scoring a large NEGATIVE selector score.
//!   4. TIES GO TO THE EARLIER PLAN, WHICH MEANS REGISTRY ORDER. Same reasoning as the candidate
//!      tie rule: predictability is the design constraint, there is no jitter anywhere in this
//!      path, and the registry authors the town doctrine first exactly as the single list collected
//!      towns first - so a town and a base of identical score still resolve to the town.
//!
//! THE COMPOSITION ROWS ARE THE POINT OF THE SECOND HALF. A plan whose selector could not pick
//! anything - every candidate blacklisted, or none of a kind it claims - is INELIGIBLE, not a plan
//! with a score of zero. The two are indistinguishable in the plan-score array and completely
//! different in effect: a zero-scored plan can still win a round in which every plan scored zero,
//! and an ineligible one never can. The rows below drive both statics in the order the runner drives
//! them, so the distinction is asserted rather than assumed.
//!
//! CAN-FAIL, four faults, injected into OVT_ObjectivePlanRules.c separately:
//!   M27. ADD INSTEAD OF MULTIPLY - `return score + priority;` in ResolvePlanScore. Compiled clean
//!        (exit 0). The case then fails on its very first row,
//!        "a multiplier of one leaves the selector score untouched: got 73.5, expected 72.5" - which
//!        is the row that exists to catch exactly this, because an addition looks almost right at
//!        small numbers and is completely wrong at large ones.
//!   M28. DROP THE PRIORITY FLOOR - the `if (priority < 0) priority = 0;` guard deleted. Compiled
//!        clean (exit 0). The case then fails on
//!        "a negative multiplier is floored to zero, not honoured: got -60, expected 0".
//!   M29. TIES TO THE LAST PLAN - `scores[i] < bestScore` in place of the `<=` short-circuit in
//!        SelectBestPlanIndex. Compiled clean (exit 0). The case then fails on
//!        "a tie between doctrines goes to the FIRST in registry order: got 2, expected 0".
//!   M30. IGNORE ELIGIBILITY - the `if (!eligible[i]) continue;` guard deleted. Compiled clean
//!        (exit 0). The case then fails on
//!        "a plan whose selector picked nothing cannot win: got 0, expected 1".
//------------------------------------------------------------------------------------------------
[Test(suite: OVT_TEST_LogicSuite, timeoutS: 30)]
class OVT_TEST_Logic_ObjectiveScaling_PlanResolution_PriorityMultipliesAndTiesByRegistryOrder : SCR_AutotestCaseBase
{
	//------------------------------------------------------------------------------------------------
	[TestStep(TestStage.Main)]
	bool Execute()
	{
		// --- Claim 1: at the shipped priority of 1 the higher raw score wins, and the rank IS the score.
		array<float> shippedScores = {72.5, 61.0};
		array<float> shippedPriorities = {1.0, 1.0};

		if (!ExpectRank(72.5, 1.0, 72.5, "a multiplier of one leaves the selector score untouched"))
			return true;

		if (!ExpectWinner(shippedScores, shippedPriorities, 0, "at equal priority the higher raw score wins"))
			return true;

		array<float> shippedReversed = {61.0, 72.5};
		if (!ExpectWinner(shippedReversed, shippedPriorities, 1, "at equal priority the higher raw score wins from the back too"))
			return true;

		// --- Claim 2: the multiplier really multiplies, and can overturn a better raw score.
		if (!ExpectRank(30.0, 2.0, 60.0, "a doubled doctrine ranks at twice its selector score"))
			return true;

		array<float> underdog = {30.0, 55.0};
		array<float> doubled = {2.0, 1.0};
		if (!ExpectWinner(underdog, doubled, 0, "a doubled doctrine out-ranks a better-scored rival"))
			return true;

		array<float> narrow = {30.0, 65.0};
		if (!ExpectWinner(narrow, doubled, 1, "a doubled doctrine does NOT out-rank a rival that is more than twice as good"))
			return true;

		// --- Claim 3: zero excludes, negative is floored to zero rather than honoured.
		if (!ExpectRank(88.0, 0.0, 0.0, "a multiplier of zero ranks the doctrine at nothing"))
			return true;

		if (!ExpectRank(60.0, -1.0, 0.0, "a negative multiplier is floored to zero, not honoured"))
			return true;

		if (!ExpectRank(-60.0, 1.0, 0.0, "a negative selector score is floored to zero as well"))
			return true;

		array<float> shelved = {40.0, 12.0};
		array<float> shelvedPriorities = {0.0, 1.0};
		if (!ExpectWinner(shelved, shelvedPriorities, 1, "a doctrine shipped at zero priority never wins, however good its target"))
			return true;

		// --- Claim 4: ties resolve to the earlier plan, which is registry order.
		array<float> tied = {50.0, 20.0, 50.0};
		array<float> flat = {1.0, 1.0, 1.0};
		if (!ExpectWinner(tied, flat, 0, "a tie between doctrines goes to the FIRST in registry order"))
			return true;

		array<float> tiedByMultiplier = {25.0, 50.0};
		array<float> compensating = {2.0, 1.0};
		if (!ExpectWinner(tiedByMultiplier, compensating, 0, "a tie produced BY the multipliers still goes to the earlier plan"))
			return true;

		// --- The composition rows: a selector that picked nothing is INELIGIBLE, not zero-scored.
		//
		// ⚠ THE HIGH RANK BELONGS TO THE INELIGIBLE PLAN ON PURPOSE. With the loser scoring higher a
		// dropped eligibility guard changes the ANSWER; with it scoring lower the row would pass with
		// the guard deleted and prove nothing at all.
		array<float> ineligibleRanks = {30.0, 14.0};
		array<bool> onlySecondEligible = {false, true};
		if (OVT_ObjectivePlanRules.SelectBestPlanIndex(ineligibleRanks, onlySecondEligible) != 1)
		{
			SetFailure("a plan whose selector picked nothing cannot win: got %1, expected 1", OVT_ObjectivePlanRules.SelectBestPlanIndex(ineligibleRanks, onlySecondEligible).ToString());
			return true;
		}

		array<bool> neitherEligible = {false, false};
		if (OVT_ObjectivePlanRules.SelectBestPlanIndex(ineligibleRanks, neitherEligible) != OVT_ObjectivePlanRules.NOTHING_TO_SELECT)
		{
			SetFailure("a round in which no doctrine could pick anything selects NOTHING: got %1, expected %2", OVT_ObjectivePlanRules.SelectBestPlanIndex(ineligibleRanks, neitherEligible).ToString(), OVT_ObjectivePlanRules.NOTHING_TO_SELECT.ToString());
			return true;
		}

		// A ZERO-SCORED but eligible plan still wins a round in which it is the only eligible one -
		// which is the distinction the flags exist to make, and the reason a selector may not say
		// "not mine" with a number.
		array<float> zeroRanks = {0.0, 14.0};
		array<bool> onlyFirstEligible = {true, false};
		if (OVT_ObjectivePlanRules.SelectBestPlanIndex(zeroRanks, onlyFirstEligible) != 0)
		{
			SetFailure("a plan that scored zero is still eligible and still wins when it is the only one: got %1, expected 0", OVT_ObjectivePlanRules.SelectBestPlanIndex(zeroRanks, onlyFirstEligible).ToString());
			return true;
		}

		// --- And the first step feeding the second: every candidate sitting out its blacklist round
		//     leaves the selector with nothing, which is what makes a plan ineligible in the runner.
		array<float> candidateScores = {80.0, 55.0};
		array<bool> allSittingOut = {true, true};
		if (OVT_ObjectiveSelection.SelectBestIndex(candidateScores, allSittingOut) != OVT_ObjectiveSelection.NOTHING_TO_SELECT)
		{
			SetFailure("an all-blacklisted candidate set leaves a doctrine with nothing to rank: got %1, expected %2", OVT_ObjectiveSelection.SelectBestIndex(candidateScores, allSittingOut).ToString(), OVT_ObjectiveSelection.NOTHING_TO_SELECT.ToString());
			return true;
		}

		Print("ObjectivePlanRules: a doctrine's rank is its selector's score times its priority, one leaves it untouched, two overturns a better target, zero and negative shelve it, and a tie goes to the earlier plan in the registry");

		return true;
	}

	//------------------------------------------------------------------------------------------------
	//! Asserts one rank, naming the row in the failure message.
	//! \param[in] selectorScore What the plan's selector scored its best candidate.
	//! \param[in] priority The plan's m_fPriority.
	//! \param[in] expected The rank the rule must produce.
	//! \param[in] label Human description of the row, used only in the failure message.
	//! \return True when it matched; false after recording the failure.
	protected bool ExpectRank(float selectorScore, float priority, float expected, string label)
	{
		float actual = OVT_ObjectivePlanRules.ResolvePlanScore(selectorScore, priority);

		if (Math.AbsFloat(actual - expected) <= OVT_TEST_LogicFixture.EPSILON)
			return true;
	...
	//------------------------------------------------------------------------------------------------
	//! Ranks every plan and asserts which one wins, exactly as the runner composes the two statics.
	//! \param[in] selectorScores One selector score per plan.
	//! \param[in] priorities One m_fPriority per plan, same order.
	//! \param[in] expected The plan index that must win.
	//! \param[in] label Human description of the row, used only in the failure message.
	//! \return True when it matched; false after recording the failure.
	protected bool ExpectWinner(array<float> selectorScores, array<float> priorities, int expected, string label)
	{
		array<float> ranks = new array<float>();
		array<bool> eligible = new array<bool>();

		int count = selectorScores.Count();
		for (int i = 0; i < count; i++)
		{
			ranks.Insert(OVT_ObjectivePlanRules.ResolvePlanScore(selectorScores[i], priorities[i]));
			eligible.Insert(true);
		}

		int actual = OVT_ObjectivePlanRules.SelectBestPlanIndex(ranks, eligible);
...
{
	//! The shipped phase NUMBERS as a deployment config authors a span of them - restated here for the
	//! reason the window case restates its bounds: a row that read a production constant could not catch
	//! that constant moving. They are ordinals into a plan's phase list, and the phase enum they used to
	//! mirror was deleted in build phase 7 (occupying/objectives) along with the last thing that read
	//! one; a config states its span as phase NAMES now, resolved against the running plan.
