...
class OVT_TEST_Logic_Town_SupportPercentage_Boundaries : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		if (emptyResult != 0)
		{
			SetFailure("SupportPercentage() with population 0 and support 0 returned %1, expected 0", emptyResult.ToString());
			return true;
		}
		...
		if (ghostResult != 0)
		{
			SetFailure("SupportPercentage() with population 0 and support 10 returned %1, expected 0", ghostResult.ToString());
			return true;
		}
		...
		if (halfResult != 50)
		{
			SetFailure("SupportPercentage() with support 25 of population 50 returned %1, expected 50", halfResult.ToString());
			return true;
		}
		...
		{
			SetFailure("SupportPercentage() with support 49 of population 50 returned %1, expected 98", nearlyFullResult.ToString());
			return true;
		}
		...
		if (fullResult != 100)
		{
			SetFailure("SupportPercentage() with support == population returned %1, expected 100", fullResult.ToString());
			return true;
		}
		...
		if (overResult != 300)
		{
			SetFailure("SupportPercentage() with support 150 of population 50 returned %1, expected 300 (there is no upper clamp)", overResult.ToString());
			return true;
		}
...
class OVT_TEST_Logic_Town_Modifiers_RecalculateSumsAndClamps : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		if (identity != 100)
		{
			SetFailure("Recalculate() with no modifiers returned %1, expected the base value 100", identity.ToString());
			return true;
		}
		...
		if (single != 95)
		{
			SetFailure("Recalculate() with one -5 modifier returned %1, expected 95", single.ToString());
			return true;
		}
		...
		if (withNull != 95)
		{
			SetFailure("Recalculate() with a null entry alongside one -5 modifier returned %1, expected 95", withNull.ToString());
			return true;
		}
		...
		if (summed != 35)
		{
			SetFailure("Recalculate() with -5 and -10 on a base of 50 returned %1, expected 35", summed.ToString());
			return true;
		}
		...
		if (clampedHigh != 100)
		{
			SetFailure("Recalculate() with a +50 modifier on a base of 100 returned %1, expected the max 100", clampedHigh.ToString());
			return true;
		}
		...
		if (clampedLow != 0)
		{
			SetFailure("Recalculate() with three -10 modifiers on a base of 10 returned %1, expected the min 0", clampedLow.ToString());
			return true;
		}
		...
		if (clampedToFloor != 5)
		{
			SetFailure("Recalculate() with a min of 5 returned %1, expected 5", clampedToFloor.ToString());
			return true;
		}
		...
		if (roundedUp != 100)
		{
			SetFailure("Recalculate() with a -0.4 effect on a base of 100 returned %1, expected 100 (rounded, not truncated)", roundedUp.ToString());
			return true;
		}
		...
		if (roundedDown != 50)
		{
			SetFailure("Recalculate() with a +0.4 effect on a base of 50 returned %1, expected 50", roundedDown.ToString());
			return true;
		}
...
class OVT_TEST_Logic_Town_SupportModifiers_DeterministicBranches : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		if (unchanged != 25)
		{
			SetFailure("Support Recalculate() with no modifiers returned %1, expected the base value 25", unchanged.ToString());
			return true;
		}
		...
		if (gained != 26)
		{
			SetFailure("Support Recalculate() with a +80 effect on a base of 25 returned %1, expected 26 (one supporter gained)", gained.ToString());
			return true;
		}
		...
		if (lost != 24)
		{
			SetFailure("Support Recalculate() with a -80 effect on a base of 25 returned %1, expected 24 (one supporter lost)", lost.ToString());
			return true;
		}
		...
		if (clampedEffect != 26)
		{
			SetFailure("Support Recalculate() with +80 and +40 (summed effect clamped to 100) returned %1, expected 26", clampedEffect.ToString());
			return true;
		}
		...
		if (clampedHigh != 50)
		{
			SetFailure("Support Recalculate() gaining a supporter at the max of 50 returned %1, expected 50", clampedHigh.ToString());
			return true;
		}
		...
		if (clampedLow != 0)
		{
			SetFailure("Support Recalculate() losing a supporter at the min of 0 returned %1, expected 0", clampedLow.ToString());
			return true;
		}
		...
		if (guarded != 25)
		{
			SetFailure("Support Recalculate() with max 0 returned %1, expected the base value 25 unchanged", guarded.ToString());
			return true;
		}
...
class OVT_TEST_Logic_Town_IsWithinTownBounds_FixedRadius : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		{
			SetFailure("IsWithinTownBounds() rejected the town's own location");
			return true;
		}

		if (!town.IsWithinTownBounds("1499 0 1000"))
		{
			SetFailure("IsWithinTownBounds() rejected a position 499 m away, inside the 500 m radius");
			return true;
		}
		...
		{
			SetFailure("IsWithinTownBounds() accepted a position exactly 500 m away - the comparison is supposed to be strictly less than the radius");
			return true;
		}

		if (town.IsWithinTownBounds("2000 0 1000"))
		{
			SetFailure("IsWithinTownBounds() accepted a position 1000 m away");
			return true;
		}
		...
		{
			SetFailure("IsWithinTownBounds() accepted a position 600 m directly above the town - the distance is supposed to be three-dimensional");
			return true;
		}
...
class OVT_TEST_Logic_Town_AreaHeat_ClampsNegativeToZero : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		{
			SetFailure("A fresh town's area heat is %1, expected 0", town.GetAreaHeat().ToString());
			return true;
		}
		...
		{
			SetFailure("SetAreaHeat(12.5) then GetAreaHeat() returned %1", town.GetAreaHeat().ToString());
			return true;
		}
		...
		if (!OVT_TEST_LogicFixture.FloatEquals(town.GetAreaHeat(), 0))
		{
			SetFailure("SetAreaHeat(-5) left the area heat at %1, expected it clamped to 0", town.GetAreaHeat().ToString());
			return true;
		}
		...
		{
			SetFailure("SetAreaHeat(1000) stored %1 - an upper clamp appears to have been added, which this case pinned as absent", town.GetAreaHeat().ToString());
			return true;
		}
...
class OVT_TEST_Logic_Town_CopyFrom_CopiesRecordButNotLocation : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		{
			SetFailure("CopyFrom() left population at %1, expected 1234", destination.population.ToString());
			return true;
		}

		if (destination.targetPopulation != 4321)
		{
			SetFailure("CopyFrom() left targetPopulation at %1, expected 4321", destination.targetPopulation.ToString());
			return true;
		}

		if (destination.stability != 42)
		{
			SetFailure("CopyFrom() left stability at %1, expected 42", destination.stability.ToString());
			return true;
		}

		if (destination.support != 77)
		{
			SetFailure("CopyFrom() left support at %1, expected 77", destination.support.ToString());
			return true;
		}

		if (destination.faction != 3)
		{
			SetFailure("CopyFrom() left faction at %1, expected 3", destination.faction.ToString());
			return true;
		}

		if (destination.gunDealerPosition != source.gunDealerPosition)
		{
			SetFailure("CopyFrom() left gunDealerPosition at %1, expected %2",
		...
		{
			SetFailure("CopyFrom() left area heat at %1, expected 9.5", destination.GetAreaHeat().ToString());
			return true;
		}

		if (destination.stabilityModifiers.Count() != 1 || destination.supportModifiers.Count() != 1)
		{
			SetFailure("CopyFrom() left %1 stability and %2 support modifiers, expected 1 of each",
		...
		{
			SetFailure("CopyFrom() appears to deep-copy the modifier arrays now (destination has %1 entries after the source grew to 2) - this case pinned them as shared references", destination.stabilityModifiers.Count().ToString());
			return true;
		}
		...
		{
			SetFailure("CopyFrom() overwrote location with %1 - it is supposed to leave the destination's own location alone", destination.location.ToString());
			return true;
		}

		if (destination.size != OVT_TownSize.VILLAGE)
		{
			SetFailure("CopyFrom() overwrote size - it is supposed to leave the destination's own size alone");
			return true;
		}
...
class OVT_TEST_Logic_Town_ModifierLimit_NonStackableIsOne : SCR_AutotestCaseBase
{
	...
	bool Execute()
	{
		...
		if (notStackable != 1)
		{
			SetFailure("A non-stackable modifier authored with stackLimit 5 reported a limit of %1, expected 1", notStackable.ToString());
			return true;
		}
		...
		if (stackableThree != 3)
		{
			SetFailure("A stackable modifier with stackLimit 3 reported a limit of %1, expected 3", stackableThree.ToString());
			return true;
		}
		...
		if (stackableOne != 1)
		{
			SetFailure("A stackable modifier with stackLimit 1 reported a limit of %1, expected 1", stackableOne.ToString());
			return true;
		}
		...
		if (outOfRange != 0)
		{
			SetFailure("An index outside the config reported a limit of %1, expected 0", outOfRange.ToString());
			return true;
		}
		...
		if (negativeIndex != 0)
		{
			SetFailure("A negative index reported a limit of %1, expected 0", negativeIndex.ToString());
			return true;
		}
		...
		if (unknownName != -1)
		{
			SetFailure("GetModifierSpaceByName() for an unknown name returned %1, expected -1", unknownName.ToString());
			return true;
		}
		...
		{
			SetFailure("GetModifierSpaceByName() with no town list returned %1, expected the full limit 1", spaceWithoutTowns.ToString());
			return true;
		}
