...
- `Configs/Map/MapOverthrow.conf` — the **delta override at vanilla's GUID** (the whole extension mechanism)
- `Prefabs/Characters/.../Character_Player.et:37-43` — context config (layout, chip colors)

## Important Decisions

- **Extension by config delta at the vanilla GUID** — Overthrow's modules/components are _appended_ to vanilla's map config. Don't replace the file; don't add a custom map entity.
...
- **Threat grid ships disabled** — re-enabling requires threat replication (server-only today → empty for clients) _and_ fixing the 2601-allocations-per-frame draw.
...
- `OVT_MapIcons`' three parallel arrays must be appended atomically _after_ widget creation succeeds; any `continue`/skip desyncs everything after it (BUG-068).
...
_This context file was created retrospectively by analyzing existing code._
