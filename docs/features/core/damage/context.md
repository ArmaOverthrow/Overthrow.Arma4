...
**Last Updated:** 2026-08-22 (post-close fix 2: the BD31 numbers had never reached the Barracks or the Warehouse — both added after the 2026-08-21 fix from the pre-fix template; repo-wide gate added)
**Current Phase:** Closed (post-close fix 2 landed 2026-08-22; play-test of both post-close fixes owed)
**Status:** ✅ Complete — play-test green 2026-08-20; **post-close fixes 2026-08-21 and 2026-08-22 landed, two play-test items owed (see Needs human verification)**
...
**Post-close fix 2 (2026-08-22):** the user saw a built **Barracks** ruined by a single stray small-arms round. The 2026-08-21 fix was correct but **incomplete by enumeration** — it edited the eight buildables that existed on 2026-08-20, and two destructible buildables were authored *after* it from the pre-fix template: `OVT_Warehouse.et` (`logistics/resources`, ba5bd37a) and `OVT_Barracks.et` (construction-site work, still untracked). Both shipped 100 000 + 100 000 with no `DamageThreshold` — the BD31 fault verbatim. Both are now 32 000 + 32 000 with `DamageThreshold 50`. ⚠ **The safety net already existed and was simply not run:** Init case F enumerates the LIVE buildables config, and both prefabs are listed in `Configs/Resistance/buildables.conf` (`:138`, `:170`), so F would have gone red on "rifle rounds chip" for each of them. The v1.5 suite run has been owed since before either was added. The added `tools/check-destructible-health.py` (BD34) is a *cheap* second net, not the missing one: it is a file scan needing no world, so it can run in seconds on any prefab edit, where F needs a loaded campaign. Init case E, by contrast, could never have caught this — it spawns one hard-coded prefab.
...
### BD34 — Prefab-authored damage numbers get a repo-wide file gate, not a per-prefab test case (2026-08-22, post-close fix 2)
BD31's fix was applied prefab by prefab to the eight buildables that existed that day, and Init case E pins the numbers on **one** hard-coded prefab (`OVT_RecruitmentTent.et`). Neither mechanism can see a buildable added next week — and two were, both copying the pre-fix template, both shipping the exact fault BD31 had just closed. The gate is therefore a **file scan, not a spawn test**: `tools/check-destructible-health.py` walks every `.et` under `Prefabs/` and `PrefabsEditable/`, and for each one carrying `OVT_StructureDestructionComponent` asserts (a) `m_fBaseHealth + Σ m_fPhaseHealth ≤ 65 535 − 1 535 margin` and (b) a `DamageThreshold` on the hit zone, with the Fuel Depot exempt from (b) by name (BD33). Coverage follows the filesystem, so it cannot go stale by omission. Proven able to fail: the reverted Barracks trips both findings. Run it after touching any destructible prefab.

### BD35 — A ruin hides the structure's children; a repair restores exactly the ones it hid (2026-08-22)
User report: a ruined barracks left its furniture and interior lights floating in the rubble. A phase change swaps the **root's** mesh and nothing else, so every child entity survives the collapse untouched. `OnBecameRuin()`/`OnBecameIntact()` now walk the child subtree and flip `EntityFlags.VISIBLE | TRACEABLE`. Two things make it safe rather than a blanket flip: **only children that are visible at the moment of the ruin are recorded**, so a repair cannot reveal something deliberately pre-hidden (vanilla's pre-hidden cargo-bed slots are exactly that shape); and the walk runs a **second pass 500 ms later** (`CHILD_HIDE_RETRY_MS`), because slotted children can spawn after the phase change lands — a stream-in is the usual case. The pass is additive and idempotent: anything already hidden is no longer `VISIBLE`, so it is skipped. This runs on **every machine** — visibility flags are local render state, not replicated — which the phase funnel (BD27) already guarantees. Side benefit: it also fixes the Bunkers composition, whose intact sandbags **are** children of a root with no mesh of its own, so the rubble used to appear inside them. Pinned by Init case G.

### BD36 — The ruin fire damages characters itself; vanilla has no area fire (2026-08-22)
User report: AI stand in a burning ruin unharmed. Confirmed in the vanilla source — there is no area-fire mechanism at all. `FireDamageSystem` ticks `UpdateFireDamage()` on the damage managers of entities that are **themselves alight**, and `SCR_FlammableHitZone` burns its own entity plus `m_CompartmentManager.DamageOccupants()` (`:493`) — occupants of a vehicle, never bystanders. Our ruin fire was a pure particle effect, so it was cosmetic by construction. `StartFireHazard()` now runs alongside the retained fire on the **authority only**, ticking once a second: a `QueryEntitiesBySphere` (DYNAMIC) at the ruin's world-bounds centre, then `SCR_CharacterDamageManagerComponent.DamageRandomHitZones(dps, EDamageType.FIRE, …)` on every living `ChimeraCharacter` inside `m_fFireDamageRadius`. Defaults 5 m / 12 dps, both prefab attributes; 0 on either disables the hazard. It is bound to the fire's own lifetime — `StopRuinFire()` stops it, so it ends with the 120 s burn, a repair, or the entity's deletion.
...
- ⚠ **A destruction phase change swaps the ROOT's model only.** Children survive it, visible and traceable, and must be hidden by hand (BD35).
- ⚠ **Vanilla has no area fire.** A burning particle effect damages nobody; bystander damage has to be dealt by script (BD36).

- 🔴 **A green safety net you never run is not a safety net.** Init case F would have failed on both new prefabs the day they landed; the suite had not been run since. A file-scan gate (BD34) is worth having next to it precisely because it costs seconds and needs no world.
- 🔴 **A fix applied by enumerating today's prefabs decays the moment another feature adds one.** BD31 was fixed on eight prefabs on 2026-08-21; by 2026-08-22 two more destructible buildables existed, both copied from the pre-fix template, both one-shot killable. Prefab-authored invariants need a filesystem-wide gate (BD34), not a per-prefab test case.
...
- [ ] **Post-close fix 2 (2026-08-22, BD34)** — empty a magazine into a built **Barracks** and a built **Warehouse**: no phase change, no ruin, `/repair-structure` not offered. These are the two prefabs the 2026-08-21 fix never reached.
- [ ] **Post-close fix 3 (2026-08-22, BD35/BD36)** — **(a)** `/ruin-structure` a **Barracks**: the furniture and the interior lights go with it, nothing floats above the rubble; `/repair-structure` brings them back. **(b)** the same on a built **Bunkers** — the rubble must no longer sit inside intact sandbags. **(c)** stand in a burning ruin's flames: you take fire damage within ~5 m and stop taking it when you step out or when the 120 s burn ends; walk an AI into it and it is hurt too. **(d)** on a listen host + one client, both (a) and (b) look the same on both machines.
