...
**Epic:** core
**Status:** 🟢 3/5 documented (legacy); persistence BUILT — SP play-test green, MP/dedicated half outstanding; controller-migration 📋 planned & scoped
...
| 6 | player-groups | 📝 Planned (requirements only) | — | Player squads: your own private group by default, opt-in joining of another player's group through vanilla's request/approve flow, a warned Group Menu (explanatory text + pre-join confirmation dialog), and the **recruits-follow-their-owner** rule — a group leader commands your recruits while you are in their group, but ownership (dismiss/rename/inventory/loadout/XP) never transfers and they return with you when you leave, switch groups or reconnect. Addresses the second half of issue **#147**; blocked on the **BUG-088** fix (players had no faction, group or leadership in any replicated session). |
...
6. **player-groups** — sits on `game-mode`'s spawn logic (where the per-player group is created) and on `resistance/recruits` (whose agents follow their owner between groups). Independent of `controller-migration`, but any new client→server call it needs belongs on `OVT_OverthrowController`, not the deprecated comms component. Cannot start until the **BUG-088** fix is play-test-verified on a dedicated server — grouping is untestable while nobody has a group.
