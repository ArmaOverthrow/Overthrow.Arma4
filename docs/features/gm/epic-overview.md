...
| 4 | waypoint-viz | 🔴 REOPENED 2026-08-23 — **root-caused: the visual is built on `Shape.Create*`, which is Workbench-only debug geometry and paints nothing in a shipped client.** Needs a Phase 6 rework onto a `CanvasWidget` (`ProjWorldToScreenNative` + `LineDrawCommand`). Wire/walk/logic all positively confirmed good against a real dedi server. The 2026-08-16 "dedi verified" claim was retracted: dedi testing had not started. All 190 green ×3. **Phase 6 planned 2026-08-24 (9 tasks): rework onto a CanvasWidget.** | 28/38 | Read-only visualization of Overthrow-generated AI waypoints in GM view (own implementation; not the E_ waypoint set) |
