...
**Status:** 🟡 In Progress (2/4 features built) — `ui` play-tested green and `storage` built with all gates green, both 2026-08-21; `resources` and `building-repair` still at requirements
...
- **Rollup status:** In Progress (2/4 features built, 122/124 tasks across the epic) — `ui` play-tested green 2026-08-21 and `storage` built the same day with every automated gate green; `resources` and `building-repair` are formalized requirements awaiting `/plan-feature`
