...
**Epic:** logistics
**Status:** 📋 Planned (0/4 features) — `ui`, `storage` and the merged `resources` requirements all formalized 2026-08-20; `building-repair` revised
...
- **Rollup status:** Planned (0/4 features) — `ui`, `storage` (moved in from `core`) and `resources` (five features merged) formalized 2026-08-20; next step `/plan-feature logistics/ui`
- **One-line summary for master:** Mid-game supply sub-game — a shared transfer UI and a type+quantity item storage ledger, then a config-driven multi-resource economy hauled by truck, dropped as crate piles, traded at the port and consumed to construct, store and repair buildings.
