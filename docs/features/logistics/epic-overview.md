...
**Epic:** logistics
**Status:** 🟡 In Progress (1/4 features built) — `ui` built and **play-tested green** 2026-08-21; `storage`, `resources` and `building-repair` still at requirements
...
- **Rollup status:** In Progress (1/4 features built, 43/44 tasks across the epic) — `ui` play-tested green 2026-08-21; the other three are formalized requirements awaiting `/plan-feature`
- **One-line summary for master:** Mid-game supply sub-game. The shared List + Cart + Destination transfer screen is **built and play-tested green** — the port Import and warehouse Take screens are now thin subclasses of one base with one layout and one `ActionContext`, and the old screens are deleted (play-tests, the string-table re-export and a wiki pass are owed). Next: the `storage` item ledger, then the multi-resource economy hauled by truck, traded at the port and consumed to construct, store and repair buildings.
