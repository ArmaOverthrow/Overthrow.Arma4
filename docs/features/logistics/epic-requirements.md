...
- **Resource costs are difficulty-driven, like money already is.** The construction requirement uses a new `OVT_DifficultySettings` multiplier in the `Economy` category applied through an `OVT_OverthrowConfigComponent` accessor shaped like `GetBuildableCost()`; the repair fraction reuses `core/damage`'s `repairCostMultiplier`, stacked on construction's scaled figure.
...
- **Repair costs resources.** `core/damage` (In Progress) makes destroyed Overthrow buildings stay in place as ruins, disables them, persists the phase and repairs them for difficulty-scaled money; this epic adds a resource requirement — the buildable's requirement × `repairCostMultiplier`, drawn from nearby crate piles — to that repair seam. No separate repair mechanic, no site prefab at the ruin.
...
- **Everything persists.** Resource stock in truck stores, crate piles, warehouses and half-finished construction sites survives save/load through the vanilla persistence system, (a building's ruin phase is persisted by `core/damage`'s `OVT_BuildableComponentSerializer` v2 — nothing new here).
...
> Revised 2026-08-20: `ui` and `storage` added (the latter moved in from `core`), and the five resource features merged into one `resources` feature. Per-feature requirements live in each subfolder.

1. **ui** — Shared List + Cart + Destination base context with the shop's tool header; replaces the port Import and warehouse Take screens; no new server wiring. — First: no intra-epic dependency.
2. **storage** — Type+quantity item ledger on ammo boxes, trucks, civilian cars and the warehouse; batched conversion to/from vanilla inventory; Open Storage screen as a `ui` consumer; port item Export. — Settles the item side before the resource ledger is designed beside it.
3. **resources** — `resources.conf` + manager + m³-capped ledger with persisted drifting prices; cargo store on every truck; merging crate piles with inspect/HUD/markers; resource Import/Export on the shared port screen with war-biased drift and difficulty multipliers; resource requirements + construction sites + Build-from-piles; warehouses hold resources and become a resource-costed buildable. — Phase order inside the feature: engine → transport → trade → construction → warehouse.
4. **building-repair** — A resource gate on `core/damage`'s repair seam: repairing a ruin also needs the buildable's requirement × `repairCostMultiplier` in nearby piles, consumed on repair, with the ruin showing needed-vs-nearby. — Last; needs `core/damage` and `resources`.

## Dependencies

- **economy epic** — `OVT_EconomyManagerComponent` (player money, prices, the port purchase path), `OVT_RealEstateManagerComponent` / `OVT_WarehouseData` (the warehouse that `resources` extends).
- **towns epic** — town control state, read by `resources` for the illegal import/export gate and for where a warehouse may be built.
...
- **map epic** — marker plumbing reused for crate piles in `resources`.
