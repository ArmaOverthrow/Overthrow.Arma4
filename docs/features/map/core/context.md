...
- **`UpdateInfoPanel` no-ops entirely when `m_InfoLayout` is empty** (`OVT_MapLocationType.c:126-127`). That is the mechanism behind "7 of 10 types show only the generic panel" — they are not falling back to a generic renderer, they are simply contributing nothing to `ContentSlot`.
