...
- \[x\] ✅ Config binding (`MapOverthrow.conf` delta + `OverthrowMap.conf`)
...
- [x] **BUG-133** (medium) — D1: icon zoom-sizing inert. **Two stacked faults**, not the one-liner the bug predicted: the lookup name was wrong (`"IconLayout"` → `"IconContainer"`, now the `ICON_CONTAINER` constant) _and_ `FrameSlot.SetSize` could never have sized it, because `IconContainer` is in a `LayoutSlot`. Now uses `SizeLayoutWidget.SetWidthOverride`/`SetHeightOverride`.
...
_This task file was created retrospectively. Add new tasks here when working on enhancements._
