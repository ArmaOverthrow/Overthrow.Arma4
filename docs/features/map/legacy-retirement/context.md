...
| Working tree                                     | clean at **`28c2f957`** | _(fix) various map/core bugs_ |
...
- `Configs/Map/MapOverthrow.conf` — the `OVT_MapIcons` block only (P3.2)
...
- `m_DescriptorDefaultsConfig` in `MapOverthrow.conf` — `map/location-types`' bus-stop duplicate suppression, nothing to do with `OVT_MapIcons`
...
`m_bFastTravelActive`, so after Phase 1 every legacy mode is _provably_ unreachable before one line of
...
### K-8 — `MapOverthrow.conf` is a same-GUID **delta** over vanilla's
...
> `OVT_MainMenuContext.c:108`'s `SetFocusedWidget(comp.GetRootWidget())` sits _inside_ the `if (comp)`
...
> layout** (see Decision D-5). _Lesson: a `SetFocusedWidget` target is a layout-order fact, not a
> code-order fact._
...
> feature's job, but worth filing. _Lesson: a `file:line` in a comment is a reference with a human
> reader instead of a compiler, and nothing ever re-checks it._
...
> part of its named scope, leaving exactly two — so the grep will pass, but _by accident, not because
> the file is correct_. Both survivors are factually stale: the live refusal is `#OVT-NotAtBusStop`
...
> registry" _without naming the class_, so it never appears in the grep at all. The true post-P3.7 set
...
> easy, because `OVT_MapThreatGrid` is one of the modules this feature deliberately _retained_.
...
> cited `RplSave/RplLoad at :780/:810` when they are at `:794`/`:824`; that one _was_ in a block being
...
### D-5 — Main-menu rows reordered by frequency of use _(user-directed, 2026-08-10)_
...
wrong once — it named a 4-line range _inside_ the 18-line `OVT-NeedBusStop` `CustomStringTableItem`,
...
FINDING A is exactly this failure mode: a `SetFocusedWidget` call hiding inside a block that _looks_
...
- **7c — two-client MP/JIP.** Two clients on `launch-server.sh`, B joining _after_ A had accumulated
...
- **7e — save compatibility.** A save created _before_ these changes loads with all markers rendering.
...
- **Two non-legacy inhabitants found in deleted blocks, both proven inert:**
  1. `OnMapExit` called the base-class `CloseLayout()` and was subscribed to the _static_
...
- ⚠️ **Intermediate state, closed by Phase 3:** `Character_Player.et` now sets four attributes whose
  script members no longer exist. Expected — but it means the plan's claim that _"the tree is shippable
  at every phase boundary"_ does **not** hold between P2 and P3. Do not open that prefab in the
...
- **The legacy map is gone.** Deleted `OVT_MapIcons.c` (846 lines), `MapIcon.layout`, `MapInfo.layout`,
  ...
  Edited the `OVT_MapIcons` block out of `MapOverthrow.conf` and the four dead attributes out of
  `Character_Player.et`.
- Gate: Q-4 grep **8 mentions / 5 files** (see FINDING G); Q-5 GUID grep **empty** repo-wide outside
  `docs/`; `MapOverthrow.conf` braces 24/24 with all keepers intact; `.et` braces 140/140; compile
...
- **The eight `OVT_MapIcons.c:<line>` pointers were all accurate** — each was resolved against the live
  file _before_ deleting it, so every preserved rationale is factually correct.
...
- Gate: RPC symbol list **61 → 59** with a diff showing exactly the two expected `RpcAsk_` names and
  ...
  `junit.xml` produced — so the scripts genuinely _loaded_, not merely compiled.
- **Safety proven before deletion, not asserted.** The old recruit ring-placement loop and
  ...
  _non-loop_ half unprompted — persistent-ID resolution, null-actor guard, `originPos` captured before
...
#### Three errors found _in the plan itself_ (all in `implementation.md` §4 P5)
...
- **Decision: rewrite both entries, keep both headers.** Deleting them would have removed the only
  in-game explanation of markers, info panels and fares — and the map is _more_ capable than the thing
...
- **Six sentences were cut because they could not be proven** — this is the valuable half:
  - _"Click the panel's close button"_ — `SetupCloseButton` exists, but **BUG-134** is an open
    widget-name mismatch for exactly that button. Documented the empty-map click instead, which was
    verified.
  - _"…including any intel known about the area"_ (from the old text) — no intel field is surfaced by
    any panel builder. Dropped.
  - _Points of interest as a fast-travel destination_ — `OVT_MapLocationPOI` has no `CanFastTravel`
    override and no `m_bCanFastTravel 1`. It is a marker only.
  - _"Travel is free in debug mode"_ — true, but a dev switch, not player help.
  - _Any concrete price or distance_ — all come from `m_Difficulty` at runtime; the old text's "a small
    fee" was an unprovable characterisation.
  - _"Markers appear as you zoom in"_ — zoom-gating exists, but no rule precise enough to state.
...
changelog**, so it stays. If the grep must come clean, the correct fix is a _new_ version entry, not an
edit to the old one.
...
1. **No commits.** `implementation.md` §4 and **Q-10** require a commit per phase, and the plan's whole
   ...
   git state, so **all seven phases are one uncommitted working-tree change**. Q-10 is therefore _not
   satisfiable by this run_ — the user should commit per phase if they want the revert path.
...
- **Export regeneration verified in the tree, not merely reported**: all six `.conf` files show a
  ...
  This mattered — until it happened, the Field Manual would have rendered the _old_ English text and
...
- **What this gate proves that nothing automated could.** Four of seven phases edited `.layout`,
  `.conf`, `.et` and `.st` files — invisible to `compile-check.sh` _and_ to both test groups. A
