...
- [x] ✅ **Done 2026-09-10**: Published the modder wiki page (Phase 8 T8.3) at `development-documentation/features/objective-plans` (page id 64). No existing objective page was found (searched `objective`, `counter attack`, `occupying`, `doctrine`). The difficulty-field table was re-verified against the shipped `.conf` files first. `objectiveHarassmentIntervalMinutes` and `objectiveHarassmentMaxConcurrent` had both drifted since the draft was written on 2026-08-21 (commits `22207f5c` and `519b8238`, 2026-08-23/24), and a new field `objectiveFirstOperationDelayMinutes` was added after the draft. The page carries the corrected table. See the `wiki-draft.md` numbers for the stale ones. Lint score 3.62/100w (target 2.5) after two passes. Dense field-reference tables carry most of the remaining passive-voice and long-paragraph flags, and 16 of the flagged "contractions" are possessive apostrophes (`plan's`, `faction's`), not real contractions.
- [x] ✅ **Done 2026-09-10** — Fixed two wrong numbers in the counter-attacks wiki brief before publishing it (`docs/features/occupying/counter-attacks/context.md` T10.3): the muster window is **fifteen** real minutes (900000 ms), not thirty, and `objectiveQRFResourceGate` is 750/750/1200/2000/3000, not 2000/1500/1200/1000/800. Checking every other number in the brief the same way found five more that had drifted; see the counter-attacks `context.md` note dated 2026-09-10 for the full corrected table and citations.
