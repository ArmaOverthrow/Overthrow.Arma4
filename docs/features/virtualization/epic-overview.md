...
| 4 | integration | Planned | — | First tracked-group consumers: deployments' group lifecycle (town patrol + both vehicle patrols) and radio-tower garrisons migrate onto the layer; ad-hoc proximity code retired; dead members stay dead in the live game. **The auto-adoption seam is now live** (`movement` Phase 3): a group registered at *any* position is adopted by the next tick pass — no notification, no callback, no registration argument — and a group a consumer `SetPosition`s re-derives from its new position. Read `virtualization/movement/context.md` → "For `integration`" before planning. |
...
- **Rollup status:** In Progress (3/5 features complete — core ✅ 50/50, civilians ✅ 39/39, movement ✅ 22/22 play-test passed)
- **One-line summary for master:** The unified AI virtualization layer (issue #100, a thin registry over Reforger 1.8's engine-native group lifecycle): **core, civilians and movement built** — registry/API + survivor masks + frozen contract, town-civilian ambience, and now infantry-only virtual movement COMPLETE incl. user play-test (2 s round-robin tick walks dormant groups along their plans; stateless resume + live-handoff direction; 3 play-test fixes landed incl. the Manual-policy spawn guard; suites green Fast 190/All 236) — next `integration` (auto-adoption seam is live), then the deferrable base-upgrades→deployments migration.
