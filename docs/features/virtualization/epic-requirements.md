...
- **AI observers (added 2026-08-17, user):** designated resistance AI groups — recruit slave groups and future high-command groups — can be registered as **observers**, so enemy AI materialises around them even with no player nearby (AI-vs-AI engagements happen live, not virtually). Mechanism: the engine's `ObserversSystem` entity-following temporary observers (`InsertObserverSP(key, x, z, pEntity)` with a **non-null** entity — the null-entity SP insert is a proven client-freezer, see core `context.md` gotcha 0), keyed per group, following the group entity (valid dormant or spawned), removed on wipe/unregister. A self-observing group also pins itself spawned — the intended lever for "mission-active" high-command groups. Landing zone: additive core API (`api.md` freeze permits additive) built in `integration` (or alongside high-command work), never hand-rolled proximity.
- Despawned **infantry** groups keep moving believably: straight-line fixed speed, with valid (ground, never water) spawn placement when players approach. **(Amended 2026-08-17, user):** vehicle groups are never virtually moved — they stay spawned (huge `spawnDistanceOverride`) and live AI drives real roads; the engine has no script route-finding API, and insertion/extraction (QRF/deployments deliver a group live, then it enters normal virt) makes vehicle transit live-spawned by design.
...
3. **movement** — Virtual movement engine (infantry straight-line only, per the 2026-08-17 amendment) + spawn placement + live-AI waypoint handoff + the insertion/extraction seam design — depends on core's records; must precede tracked-group consumers so migrated groups don't freeze while despawned.
