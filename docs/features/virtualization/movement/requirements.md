...
- **Handoff both directions:** largely native under the 1.8 model (2026-08-14 replan) — waypoint entities stay attached to the dormant group across despawn/respawn, so movement's job is advancing the dormant group entity's **origin** (via core's `SetPosition`) along its waypoint route; on materialisation the engine spawns members at that origin and live AI resumes the same waypoints. Route progress bookkeeping (current leg, interpolation) is movement's own state.
...
- `virtualization/core` must be complete — this feature is a tick strategy over core's registered dormant group entities (writable position, owned waypoints; core `implementation.md` Revision 2).
